...
作成日: 2026-05-10
更新日: 2026-05-10（[1] モデルアクセス有効化ステップを削除）
...
Phase 1 で作成した `handson-ec2-role` に Bedrock へのアクセス権限を追加する。
...
**実行場所: `~/samurai-repo`**（[2] の続き。すでに root かつ git pull 済みの前提）
...
**確認 2: IAMロールに権限があるか**
- IAM コンソール → ロール → `handson-ec2-role`
