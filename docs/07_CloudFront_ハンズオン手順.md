...
## [3] （任意）WAF で日本からのアクセスをブロックする（動作確認用）

CloudFront に WAF（Web Application Firewall）を設定し、**特定の国（ここでは日本）からのアクセスをブロック**する。
WAF が正しく動作しているかをブラウザで確認できる。確認後は必ず削除すること。
...
**ステップ 2: ルールを追加する（国別ブロックルールを作成）**

1. 「**自分のルールとルールグループを追加する**」をクリック
2. 「**ルールを作成**」を選択（マネージドルールグループではないので注意）
3. 以下を設定する：
...
| 項目 | 値 |
|------|-----|
| 検査対象 | 「発信元の国」（Originates from a country in） |
| 国コード | `JP` を選択（リストから Japan / JP を探す） |
...
4. 「ルールを追加」をクリック → リストに `block-japan` が追加されたことを確認 → 「次へ」
...
Web ACL の作成後、CloudFront ディストリビューションへの反映には数分かかる。

**WAF の関連付けを確認:**
- WAF コンソール → 「Web ACLs」→ `handson-webacl` を選択
- 「関連付けられた AWS リソース」タブ → `handson-cloudfront` が表示されていれば OK

**ブラウザで WAF が動いているかを確認:**

ブラウザで `https://[CloudFrontドメイン]` にアクセスする。

日本のIPからアクセスしているため、以下のエラーが表示されれば **WAF が正常に機能している**。

```
403 Forbidden
```

> CloudFront / WAF が返す 403 画面（英語）が表示される。アプリのログイン画面ではなく、
> シンプルなエラー画面が出ればブロック成功。

---

### 3-5. 動作確認後はルールを無効化 or WAF を削除する

ブロックを確認したら、アプリを使えるように戻す。以下のどちらかを選ぶ。

**方法 A: ルールを無効化してアプリを使い続ける（WAF を残したまま）**

1. WAF コンソール → `handson-webacl` → 「ルール」タブ
2. `block-japan` のトグルを **オフ**（無効）にする
3. 「保存」をクリック
4. 数分後にアプリに再アクセスできるようになる

**方法 B: WAF ごと削除する（ハンズオン終了時）**
...
| WAF Web ACL | **$5/月** | Web ACL 1 つにつき固定。**ハンズオン後は削除すること** |
