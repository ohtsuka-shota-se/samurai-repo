...
Metric Filter で作成したメトリクスが閾値を超えたら SNS に通知するアラームを作成する。

1. CloudWatch コンソール → **アラーム** → **アラームの作成** をクリック
2. **メトリクスの選択** → `HandsonApp` → `PM2ErrorCount` を選択 → **メトリクスの選択** をクリック
3. 条件を設定：
...
6. **次へ** → アラーム名に `handson-pm2-error-alarm` を入力
7. **アラームの作成** をクリック
